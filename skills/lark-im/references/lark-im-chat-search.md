...
This skill maps to the shortcut: `xfchat_cli im +chat-search` (internally calls `POST /open-apis/im/v2/chats/search`).

## Commands

```bash
# Search chats by keyword
xfchat_cli im +chat-search --query "project"

# Restrict by search types
xfchat_cli im +chat-search --query "project" --search-types "private,public_joined"

# Filter by member open_ids (with keyword)
xfchat_cli im +chat-search --query "project" --member-ids "ou_xxx,ou_yyy"

# Search by member open_ids only
xfchat_cli im +chat-search --member-ids "ou_xxx,ou_yyy"

# Only show chats you created or manage
xfchat_cli im +chat-search --query "project" --is-manager

# Set page size
xfchat_cli im +chat-search --query "project" --page-size 10

# Pagination
xfchat_cli im +chat-search --query "project" --page-token "xxx"

# JSON output
xfchat_cli im +chat-search --query "project" --format json

# Preview the request without executing it
xfchat_cli im +chat-search --query "project" --dry-run
```
...
```bash
xfchat_cli im +chat-search --query "design review"
```

### Scenario 2: Search a chat and list recent messages

```bash
CHAT_ID=$(xfchat_cli im +chat-search --query "project" --format json | jq -r '.data.chats[0].chat_id')
...
```bash
CHAT_ID=$(xfchat_cli im +chat-search --query "daily report" --format json | jq -r '.data.chats[0].chat_id')
xfchat_cli im +messages-send --chat-id "$CHAT_ID" --text "Today's progress update"
```
...
| Permission denied (99991679) with `--as user` | UAT is not authorized for `im:chat:read` | Run `xfchat_cli auth login --scope "im:chat:read"` |
