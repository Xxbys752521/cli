...
- **Owner-only actions such as owner transfer:** require the actual owner to complete UAT authorization via `xfchat_cli auth login`, then perform the action as that owner.
...
1. Create the group with the bot first, excluding invisible users: `xfchat_cli im +chat-create --name "Group Name"`
