...
This skill maps to the shortcut: `xfchat_cli im +chat-update` (internally calls `PUT /open-apis/im/v1/chats/:chat_id`).

## Commands

```bash
# Update the group name
xfchat_cli im +chat-update --chat-id oc_xxx --name "New Group Name"

# Update the group description
xfchat_cli im +chat-update --chat-id oc_xxx --description "Updated group description"

# Update multiple fields at once
xfchat_cli im +chat-update --chat-id oc_xxx \
  --name "Q2 Project Team" \
  --description "Owns Q2 goal tracking"

# Preview the request without executing it
xfchat_cli im +chat-update --chat-id oc_xxx --name "Test" --dry-run
```
...
| Permission denied (99991679) | Missing `im:chat:update` permission | Run `xfchat_cli auth login --scope "im:chat:update"` |
