...
metadata:
  requires:
    bins: ["xfchat_cli"]
  cliHelp: "xfchat_cli mail --help"
---

# mail (v1)

> **注意**：当前 `xfchat_cli` fork 未启用 `mail` 运行时命令；本 skill 仅保留为迁移参考，不能作为可执行能力声明使用。
...
Shortcut 是对常用操作的高级封装（`xfchat_cli mail +<verb> [flags]`）。有 Shortcut 的操作优先使用。
...
```bash
xfchat_cli schema mail.<resource>.<method>   # 调用 API 前必须先查看参数结构
xfchat_cli mail <resource> <method> [flags] # 调用 API
```
