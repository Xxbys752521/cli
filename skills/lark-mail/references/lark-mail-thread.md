...
本 skill 对应 shortcut `xfchat_cli mail +thread`，内部调用：
...
```bash
# 读取完整会话
xfchat_cli mail +thread --thread-id <thread-id>

# 仅纯文本正文（更小的负载，适合 AI 处理）
xfchat_cli mail +thread --thread-id <thread-id> --html=false

# 指定邮箱
xfchat_cli mail +thread --mailbox user@example.com --thread-id <thread-id>

# JSON 输出
xfchat_cli mail +thread --thread-id <thread-id> --format json

# Dry Run
xfchat_cli mail +thread --thread-id <thread-id> --dry-run
```
...
```bash
xfchat_cli mail +thread --thread-id <thread_id> --format json | jq -r '.data.messages[0].body_html'
```
...
```bash
# 1. 从某封邮件获取 thread_id
xfchat_cli mail +message --message-id <id> --html=false --format json | jq '.data.thread_id'
...
# 回复
xfchat_cli mail +reply --message-id <last_message_id> --body "..."
```

## 相关命令

- `xfchat_cli mail +message` — 读取单封邮件
...
- `xfchat_cli mail user_mailbox.messages list` — 列出收件箱邮件（获取 `thread_id`）
