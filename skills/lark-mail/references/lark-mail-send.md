...
本 skill 对应 shortcut：`xfchat_cli mail +send`。
...
```bash
xfchat_cli mail +send --to <收件人> --subject '<主题>' --body '<正文>'
```
→ 返回 `draft_id`
...
# 保存为草稿并抄送
xfchat_cli mail +send --to alice@example.com --cc bob@example.com --subject '状态更新' --body '<b>已完成</b>'
...
# 保存带附件的草稿
xfchat_cli mail +send --to alice@example.com --subject '请查收' --body '<p>见附件</p>' --attach ./report.pdf,./logs.zip

# 保存带内嵌图片的草稿（CID 为唯一标识符，可用随机字符串）
xfchat_cli mail +send --to alice@example.com --subject '预览图' --body '<img src="cid:a1b2c3d4e5f6a7b8c9d0">' --inline '[{"cid":"a1b2c3d4e5f6a7b8c9d0","file_path":"./logo.png"}]'

# 纯文本邮件（仅在内容极简时使用）
xfchat_cli mail +send --to alice@example.com --subject '确认' --body '收到，谢谢'

# Dry Run（仅打印请求，不执行）
xfchat_cli mail +send --to alice@example.com --subject '测试' --body '<p>test</p>' --dry-run
```
...
```bash
xfchat_cli mail +send --to alice@example.com --subject '周报' --body '<p>本周进展如下...</p>'
```
...
```bash
# Step 1: 创建草稿
xfchat_cli mail +send --to alice@example.com --subject '收到' --body '<p>已收到，谢谢！</p>'
# → 返回 draft_id
