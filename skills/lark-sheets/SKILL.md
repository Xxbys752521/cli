...
metadata:
  requires:
    bins: ["xfchat_cli"]
  cliHelp: "xfchat_cli sheets --help"
---
...
Shortcut 是对常用操作的高级封装（`xfchat_cli sheets +<verb> [flags]`）。有 Shortcut 的操作优先使用。
...
```bash
xfchat_cli schema sheets.<resource>.<method>   # 调用 API 前必须先查看参数结构
xfchat_cli sheets <resource> <method> [flags] # 调用 API
```
