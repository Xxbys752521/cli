...
本文档提供基于 `xfchat_cli base ...` 的完整示例，覆盖 unified Shortcut 与当前 `base/v3` 原生 API 的常见组合方式。
...
```bash
xfchat_cli base table.fields list \
...
```bash
xfchat_cli base table.records list \
  --params '{"base_token":"bascnXXXXXXXX","table_id":"tblXXXXXXXX","limit":100}'
```
...
```bash
xfchat_cli base table.records patch \
...
```bash
xfchat_cli base table.records delete \
  --params '{"base_token":"bascnXXXXXXXX","table_id":"tblXXXXXXXX","record_id":"recXXXXXXXX"}'
```
...
```bash
xfchat_cli base view.filter update \
...
- 需要一次性建表并附带字段、视图时，优先 `xfchat_cli base +table-create`
- 需要按业务字段名做 upsert 时，优先 `xfchat_cli base +record-upsert`
- 需要配置筛选视图时，优先 `xfchat_cli base +view-set-filter`
- 需要记录历史时，优先 `xfchat_cli base +record-history-list`
