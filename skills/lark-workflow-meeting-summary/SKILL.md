...
```bash
xfchat_cli auth login --domain vc        # 基础（查询+纪要）
xfchat_cli auth login --domain vc,drive   # 含读取纪要文档正文、生成文档
```
...
```bash
# page-size 最大为 30
xfchat_cli vc +search --start "<YYYY-MM-DD>" --end "<YYYY-MM-DD>" --format json --page-size 30
```
...
1. 查询会议关联的纪要信息
```bash
xfchat_cli vc +notes --meeting-ids "id1,id2,...,idN"   
```
...
```bash
xfchat_cli docs +create --title "会议纪要汇总 (<start> - <end>)" --markdown "<内容>"
# 或追加到已有文档
xfchat_cli docs +update --doc "<url_or_token>" --mode append --markdown "<内容>"
```
