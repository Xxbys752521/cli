# `spark` 与 `xfchat_cli` 对比报告

日期：2026-03-29

## 一、结论先行

如果把 `spark` 和 `xfchat_cli` 放在同一层比较，容易得出偏差结论。  
更准确的说法应该是：

- `spark` 当前更像一个面向具体机器人产品形态的业务系统，里面包含了飞书接入、Agent 运行时、工具体系和业务逻辑。
- `xfchat_cli` 更像一个飞书能力层，把开放平台能力整理成稳定命令、身份模型和技能说明，适合被人、脚本和 Agent 复用。

从长期建设的角度看，`xfchat_cli` 这套范式更适合做“共用底座”；`spark` 更适合继续承接业务场景和产品逻辑。

这不是说现在要直接替换 `spark`，而是说新增飞书能力、私有化适配和 Agent 接入，优先往 `xfchat_cli` 收敛，会更稳。
...
而 `xfchat_cli` 的出发点相反：先把飞书能力做成 Agent 易于调用的形态，再让业务系统去消费。
...
- `xfchat_cli` 更像“先有 Agent 可以稳定调用的能力层，再让业务机器人去组合它”。

从迭代速度和灵活度看，后者通常更占优，原因有三点：

1. **不需要先定义完整场景**
   - 传统做法往往要先想清楚“用户会问什么、流程怎么走、卡在哪一步”。
   - `xfchat_cli` 这种原子能力层更适合让用户直接提要求，再由 Agent 临场组织动作。
...
从这个角度看，`xfchat_cli` 更像是在给 Agent 一套可以持续完成任务的操作面，而不是一组离散功能按钮。
...
如果看后续复用、私有化适配、Agent 接入成本和跨项目共享，`xfchat_cli` 更值得继续投入。
