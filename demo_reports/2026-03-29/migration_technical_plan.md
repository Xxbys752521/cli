...
**是否把飞书能力层从项目内分散封装，逐步收敛到 `lark-cli` 这一统一能力层。**
...
运行时只向 `lark-cli` 注入最终解析后的身份上下文，不把 token 暴露给 Agent。
...
1. 统一配置与 token 存储服务
2. `lark-cli` 运行前由 wrapper 注入对应配置视图

也就是说，保留 `lark-cli` 的身份语义和权限模型，替换其底层存储实现。
...
如果一个 Skill 的本质是“教 Agent 如何使用飞书能力”，建议迁到 `lark-cli skills` 体系。  
...
- `FeishuCapabilityToolset`：统一代理到 `lark-cli`
...
### Phase 1：加一层 `spark -> lark-cli` 适配

先不要上来就重写 Agent。  
先做 adapter：

- 对外仍保留现有工具名
- 对内改为执行 `lark-cli` 对应命令
...
每下线一类，就把 ownership 明确迁到 `lark-cli`。
...
如果只新增 `lark-cli`，但不逐步下线旧飞书封装，会出现双轨并存。  
这样会导致：
...
如果目标是让飞书能力变成多个机器人、多个 Agent、多个场景都能共用的底座，那么把飞书能力层迁到 `lark-cli` 范式，是更值得投入的方向。
